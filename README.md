...
6. **（任意）Next クライアント** — 別ターミナルで `make client-install`（初回）のあと `make client-dev`（[client/README.md](client/README.md)）。初回は `client/.env.local` が無いと `make client-dotenv` 相当で `env.example` から作成されます。
